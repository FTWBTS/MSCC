...
class Multi_CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        #     outputs = torch.zeros(1, b, self.n_features).to(self.device)
        #     x_scales_unsq = []
            
        #     for i, alpha in enumerate([self.alpha1, self.alpha2, self.alpha3, self.alpha4]):
        #         alpha.data = alpha.data.to(self.device)
...
class MSCC():
    def __init__(self,
        ...
        print("kernelsize:",self.kernel_size)
        print("dropout:",self.dropout_rate)
        print("hidden",self.hidden_activation)
